...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType) : LiveData<Course?> {
        val querySchedule = nearestQuery(queryType)
        return dao.getNearestSchedule(querySchedule)
    }

    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val queryCourse = QueryUtil.sortedQuery(sortType)
        val config = PagedList.Config.Builder()
            .setPageSize(PAGE_SIZE)
            .build()
        return LivePagedListBuilder(dao.getAll(queryCourse), config).build()
    }
    ...
    fun getTodaySchedule() : List<Course> {
        ...
        val scheduleDay = scheduleDate.get(Calendar.DAY_OF_WEEK)
        return dao.getTodaySchedule(scheduleDay)
    }
